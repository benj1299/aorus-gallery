...
export function PressListClient({ articles }: { articles: PressArticle[] }) {
  const { query, setQuery, filtered } = useSearch(articles, ['title', 'publication']);
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="h-24 text-center text-muted-foreground">
                  Aucun article de presse pour le moment. <Link href="/admin/press/new" className="underline">Cr&eacute;ez le premier</Link>.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
