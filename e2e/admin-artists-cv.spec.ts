...
/** Navigate to admin artists and search for the test artist to handle pagination */
async function findArtistRow(page: Page, name: string) {
  await page.goto('/admin/artists');
  await page.locator('input[placeholder*="Rechercher"]').fill(name);
  // Wait for the filtered table to show results
...
test.describe('Artist CV Management', () => {
  ...
  test.afterAll(async ({ browser }) => {
    ...
    try {
      const row = await findArtistRow(page, artistName);
      await row.locator('[title="Supprimer"]').click();
      await row.locator('[title="Confirmer"]').click();
      await page.waitForTimeout(3000);
    } catch {
      // Artist may not exist if tests failed early
    }
    await context.close();
  });
  ...
  test('create artist with CV entries and years', async ({ page }) => {
    ...
    // Image — switch to URL tab and paste a URL
    ...
    // Verify artist appears (search to handle pagination)
    await page.locator('input[placeholder*="Rechercher"]').fill(artistName);
  ...
  test('edit artist preserves CV years', async ({ page }) => {
    ...
    const row2 = await findArtistRow(page, artistName);
  ...
  test('remove specific CV entry removes correct one', async ({ page }) => {
    ...
    // Remove the middle entry (index 1, Tokyo 2022)
    ...
    // After removing, should have 2 entries
    ...
    // Re-open and verify only 2 entries remain
    const row2 = await findArtistRow(page, artistName);
    await row2.locator('[title="Modifier"]').click();
