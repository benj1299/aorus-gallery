...
import { httpsUrl, optionalHttpsUrl, serializeTranslatable, booleanFromString, readDimension } from '@/lib/schemas/common';
...
export async function upsertBanner(formData: FormData): Promise<{ error: string } | void> {
  ...
  if (existing) {
    await db.homeBanner.update({
      ...
      data: { ...data, subtitle: serializeTranslatable(data.subtitle), linkUrl: data.linkUrl || null, imageWidth, imageHeight },
    });
  } else {
